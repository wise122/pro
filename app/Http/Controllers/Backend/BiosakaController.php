...
class BiosakaController extends Controller
{
    protected $model;
    protected $title      = 'Biosaka';
    protected $view       = 'backend.biosaka.';
    protected $route      = 'biosaka.';
    protected $permission = '-biosaka';
    ...
    public function index(Request $request)
    {
        if (\Auth::user()->isAbleTo('manage' . $this->permission)) {
            if ($request->ajax()) {
                $data = $this->model::where('type', 'biosaka')->select();
                return DataTables::of($data)
                    ...
                    ->editColumn('image', function ($data) {
                        $url = asset('storage/biosaka/' . $data->image);

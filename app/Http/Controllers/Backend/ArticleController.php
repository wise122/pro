...
class ArticleController extends Controller
{
    ...
    protected $view       = 'backend.article.';
    protected $route      = 'inovasi-pertanian.';
            ...
            if ($request->ajax()) {
                ...
                return DataTables::of($data)
                    ...
                    ->editColumn('image', function ($data) {
                        $url = asset('storage/articles/' . $data->image);
    ...
    public function create($type)
    {
        if (\Auth::user()->isAbleTo('create'.$this->permission)) {
            ...
            return view($this->view.'create')->with(compact('type', 'category', 'tag'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $article->type = ($request->type == 1) ? 'iot' : 'biosaka';
